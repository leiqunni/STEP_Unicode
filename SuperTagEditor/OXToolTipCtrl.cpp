...
LPCWSTR COXToolTipCtrl::m_szArrowSpace = L"Å@";
...
COXToolTipCtrl::~COXToolTipCtrl()
{
    ...
    int nIndex; for (nIndex = 0; nIndex < nSize; nIndex++)
    {
...
void COXToolTipCtrl::OnPaint() 
{
    ...
    if (m_bHasExtendedText)
    {
        ...
        if (!m_bExtended)
        {
            pt[0] = CPoint(rect.right - size.cx + nXMargin, rect.bottom - nYMargin/*rect.top + nYMargin*/);
