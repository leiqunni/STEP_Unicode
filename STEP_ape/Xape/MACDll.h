...
extern "C"
{
	...
	__declspec( dllexport ) int __stdcall ShowFileInfoDialog(const char * pFilename, HWND HWndWindow);
	__declspec( dllexport ) int __stdcall TagFileSimple(const char * pFilename, const char * pArtist, const char * pAlbum, const char * pTitle, const char * pComment, const char * pGenre, const char * pYear, const char * pTrack, BOOL bClearFirst, BOOL bUseOldID3);
	__declspec( dllexport ) int __stdcall GetID3Tag(const char * pFilename, ID3_TAG * pID3Tag);
	__declspec( dllexport ) int __stdcall RemoveTag(const char * pFilename);
}
...
extern "C"
{
	__declspec( dllexport ) APE_DECOMPRESS_HANDLE __stdcall c_APEDecompress_Create(const char * pFilename, int * pErrorCode = NULL);
